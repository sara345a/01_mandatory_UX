...
const displayResult = (
  ...
  document.querySelector("#result #from_value_span").textContent =
    value + " " + getShortUnit(fromUnit);
  document.querySelector("#result #to_value_span").textContent =
    result.toFixed(2) + " " + getShortUnit(toUnit);
};
...
const handleConvertFormSubmission = (event) => {
  ...
  if (fromUnit === toUnit) {
    ...
    if (result === null) {
      displayError(
        "Sorry, an error occured while calculating!",
        errorContainer
      );
    }
    // display result
...
const inital = () => {
  ...
  document
    .querySelector("form")
    .addEventListener("submit", (e) => handleConvertFormSubmission(e));
};
