...
const formSumbited = (event) => {
  ...
  const finalTaxResult = calculateTax(moneratyAmount, taxPercentage);
  const finalResult = moneratyAmount - finalTaxResult;

  // display tax result
  const taxResultSpan = document.querySelector("#tax_result");
  taxResultSpan.textContent = finalTaxResult.toFixed(2);
